...
public class SimpleDRLTest {
    ...
    private static int numOfMobileDevices = 3;
    ...
    public static void main(String[] args) {
        try {
            ...
            for(int i=0; i<numOfEdgeServers; i++) {
                moduleMapping.addModuleToDevice("object_detector", "edge-server-"+i);
            }
            
            for(int i=0; i<numOfEdgeServers; i++) {
                for(int j=0; j<numOfMobileDevices; j++) {
                    moduleMapping.addModuleToDevice("client", "mobile-"+i+"-"+j);
                }
            }
    ...
    private static void createFogDevices(int userId) {
        ...
        for(int i=0; i<numOfEdgeServers; i++){
            ...
            for(int j=0; j<numOfMobileDevices; j++){
                ...
                Sensor sensor = new Sensor("sensor-"+i+"-"+j, "CAMERA", userId, mobileDeviceName, new DeterministicDistribution(5));
                sensors.add(sensor);
    ...
    private static FogDevice createFogDevice(String nodeName, long mips,
        ...
        try {
            fogdevice = new FogDevice(nodeName, characteristics, 
                    new AppModuleAllocationPolicy(hostList), storage, upBw, downBw, 0, ratePerMips);
